...
export function TVData({ name, poster_path, overview, vote_average, id }) {
  ...
  return (
    <div className="item">
      <div className='badge-info' style={{backgroundColor: 'black'}}>
          ...
          <div className='info-right'><Badge variant="info"  style={{backgroundColor: 'black'}}><Link to={'/info/tv/'+id+'/'+name} para style={{color: 'white', textDecoration: 'none'}}>More</Link></Badge></div>
      </div>
      <Link to={'/info/tv/'+id+'/'+name}><img src={IMAGE_API + poster_path} alt={name} class="image"></img></Link>

      <div class="outerDiv">
        <div class="CentralDiv">{name}</div>
      </div>
